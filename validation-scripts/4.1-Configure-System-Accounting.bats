...
@test "4.1.3 Ensure events that modify date and time information are collected (Scored)" {
    run bash -c "grep -r time-change /etc/audit/rules.d/"
    [ "$status" -eq 0 ]
...
@test "4.1.15 Ensure system administrator command executions (sudo) are collected (Scored)" {
    
    run bash -c "grep -h actions /etc/audit/rules.d/*.rules | grep arch=b64"
    [ "$status" -eq 0 ]
    [[ "$output" == "-a always,exit -F arch=b64 -C euid!=uid -F euid=0 -Fauid>=1000 -F auid!=4294967295 -S execve -k actions" ]]


    run bash -c "grep -h actions /etc/audit/rules.d/*.rules | grep arch=b32"
    [ "$status" -eq 0 ]
    [[ "$output" == "-a always,exit -F arch=b32 -C euid!=uid -F euid=0 -Fauid>=1000 -F auid!=4294967295 -S execve -k actions" ]]


    # log file setting has to be successfully configured for
    run bash -c "auditctl -l | grep actions | grep arch=b64"
    [ "$status" -eq 0 ]
    [[ "$output" == "-a always,exit -F arch=b64 -S execve -C uid!=euid -F euid=0 -F auid>=1000 -F auid!=-1 -F key=actions" ]]


    run bash -c "auditctl -l | grep actions | grep arch=b32"
    [ "$status" -eq 0 ]
