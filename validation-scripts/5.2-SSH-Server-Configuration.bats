...
@test "5.2.13 Ensure only strong MAC algorithms are used (Scored)" {
...
@test "5.2.14 Ensure only strong Key Exchange algorithms are used (Scored)" {
...
@test "5.2.15 Ensure SSH Idle Timeout Interval is configured (Scored)" {
...
@test "5.2.16 Ensure SSH LoginGraceTime is set to one minute or less (Scored)" {
...
@test "5.2.17 Ensure SSH access is limited (Scored)" {
...
@test "5.2.18 Ensure SSH warning banner is configured (Scored)" {
    run bash -c "sshd -T | grep banner"
    [ "$status" -eq 0 ]
    [[ "$output" == "banner /etc/issue.net" ]]
}

@test "5.2.19 Ensure SSH PAM is enabled (Scored)" {
    run bash -c "sshd -T | grep -i usepam"
    [ "$status" -eq 0 ]
    [[ "$output" == "usepam yes" ]]
}

@test "5.2.20 Ensure SSH AllowTcpForwarding is disabled (Scored)" {
    run bash -c "sshd -T | grep -i allowtcpforwarding"
    [ "$status" -eq 0 ]
    [[ "$output" == "allowtcpforwarding no" ]]
}

@test "5.2.21 Ensure SSH MaxStartups is configured (Scored)" {
...
@test "5.2.22 Ensure SSH MaxSessions is set to 10 or less (Scored)" {
