...
@test "5.4.5 Ensure default user shell timeout is 900 seconds or less (Scored)" {
    run bash -c "grep -h -E -i \"^\s*(\S+\s+)*TMOUT=(900|[1-8][0-9][0-9]|[1-9][0-9]|[1-9])\s*(\S+\s*)*(\s+#.*)?$\" /etc/bash.bashrc /etc/profile /etc/profile.d/*.sh"
    [ "$status" -eq 0 ]
    [[ "$output" = "TMOUT=900" ]]

    run bash -c "grep -P '^\s*([^$#;]+\s+)*TMOUT=(9[0-9][1-9]|0+|[1-9]\d{3,})\b\s*(\S+\s*)*(\s+#.*)?$' /etc/profile /etc/profile.d/*.sh /etc/bash.bashrc"
    [ "$status" -eq 1 ]
}
